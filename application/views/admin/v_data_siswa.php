  <!-- Content Wrapper. Contains page content -->
  <div class="content-wrapper">
    <!-- Content Header (Page header) -->
    <section class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h1>Data Siswa</h1>
          </div>
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              <li class="breadcrumb-item"><a href="#">Home</a></li>
              <li class="breadcrumb-item active">Data Siswa</li>
            </ol>
          </div>
        </div>
      </div><!-- /.container-fluid -->
    </section>

    <!-- Main content -->
    <section class="content">
      <div class="container-fluid">
        <div class="row">
          <!-- left column -->
          <div class="col-md-12">
            <!-- jquery validation -->
            <div class="card card-primary">
              <div class="card-header">
                <h3 class="btn btn-sm btn-outline-light">Data Siswa</h3>
                <h3 class="btn btn-sm btn-outline-light">Data Ortu</h3>
                <h3 class="btn btn-sm btn-outline-light">Data ....</h3>
                <h3 class="btn btn-sm btn-outline-light">Data ....</h3>
                <h3 class="btn btn-sm btn-outline-light">Data ....</h3>
              </div>
              <!-- /.card-header -->
              <!-- form start -->
              <form id="quickForm">
                <div class="card-body">
                  <div class="form-group">
                    <label for="exampleInputEmail1">No Induk/NISN</label>
                    <input type="email" name="email" class="form-control col-sm-6" id="exampleInputEmail1" >
                  </div>
                  <div class="form-group">
                    <label for="exampleInputPassword1">Nama Lengkap</label>
                    <input type="password" name="password" class="form-control col-sm-10" id="exampleInputPassword1">
                  </div>
                  <div class="form-group">
                    <label for="exampleInputPassword1">Jenis Kelamin</label>
                    <!--<input type="password" name="password" class="form-control col-sm-3" id="exampleInputPassword1">-->
                    <br>
                    <select name="level" class="custom-select col-sm-2">
                      <option>Laki-laki</option>
                      <option>Perempuan</option>
                    </select>
                  </div>
                  <div class="form-group" class="col-sm-6">
                    <label for="exampleInputPassword1">Tempat Lahir</label>
                    <input type="password" name="password" class="form-control col-sm-4" id="exampleInputPassword1">
                  </div>
                  <div>
                    <label for="exampleInputPassword1">Tanggal Lahir</label>
                    <input type="password" name="password" class="form-control col-sm-6" id="exampleInputPassword1">
                   </div> 
                  <div class="form-group">
                    <label for="exampleInputPassword1">Jurusan</label>
                    <input type="password" name="password" class="form-control col-sm-8" id="exampleInputPassword1">
                  </div>
                  <div class="form-group">
                    <label for="exampleInputPassword1">Agama</label>
                    <!--<input type="password" name="password" class="form-control col-sm-4" id="exampleInputPassword1">-->
                    <br>
                    <select name="level" class="custom-select col-sm-2">
                      <option>Islam</option>
                      <option>Kristen</option>
                      <option>Katolik</option>
                      <option>Buddha</option>
                      <option>Hindu</option>
                      <option>Kong Hu Cu</option>
                    </select>
                  </div>
                  <div class="form-group">
                    <label for="exampleInputPassword1">No Telp/HP</label>
                    <input type="password" name="password" class="form-control col-sm-6" id="exampleInputPassword1">
                  </div>
                  <div class="form-group">
                    <label for="exampleInputPassword1">Alamat</label>
                    <input type="password" name="password" class="form-control col-sm-10" id="exampleInputPassword1">
                  </div>
                  <div class="form-group">
                    <label for="exampleInputPassword1">Kewarganegaraan</label>
                    <input type="password" name="password" class="form-control col-sm-6" id="exampleInputPassword1">
                  </div>
                </div>
                <!-- /.card-body -->
                <div class="card-footer">
                  <button type="submit" class="btn btn-primary">Submit</button>
                </div>
              </form>
            </div>
            <!-- /.card -->
            </div>
          <!--/.col (left) -->
          <!-- right column -->
          <div class="col-md-6">
